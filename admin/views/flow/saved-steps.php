...
if (!empty($data['flow_steps'])):
    foreach ($data['flow_steps'] as $steps):   
        ...
        $no_settings    = (!empty($steps['no_settings']))? "data-no-settings=\"1\"" : null;
?>
    <li class="cauto-steps-el cauto-steps-el-saved cauto-step-group_<?php echo esc_attr($steps['step_group']); ?> cauto-added-step cauto_step_set_wide" >
        <div class="cauto-step-element" <?php echo $no_settings; ?> data-step="<?php echo esc_attr($steps['step']); ?>"><?php echo $steps['icon'].$steps['step_label']; ?><span class="cauto_describe_step_label"> <b><?php echo esc_html($describe_label); ?></b></span></div>
